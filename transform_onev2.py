...
DEFAULT_CONTROLLER_COLS  = ROOT / "models" / "controller_feature_cols_NOTEBOOK_ORDER.json"
DEFAULT_CTRL_LOOKUP = ROOT / "data" / "eval" / "controller_dataset_yamnet_m003.csv"
...
def _load_controller_cols(path: Path) -> List[str]:
    ...
    if not isinstance(cols, list) or not cols:
        raise RuntimeError(f"controller cols JSON invalid: {path}")
...
def _load_ctrl_lookup(csv_path: Path) -> Dict[str, Dict[str, Any]]:
...
def _merge_lookup_features(feat_dict: Dict[str, float], cols: List[str], lookup_row: Dict[str, Any]) -> None:
    if not lookup_row:
        return
    for c in cols:
        if c in feat_dict:
            continue
        if c in lookup_row:
            feat_dict[c] = _safe_float(lookup_row.get(c), 0.0)
...
def _candidate_grid() -> List[Dict[str, float]]:
    low_gain_vals = [-3.0, 0.0, 3.0]
    mid_gain_vals = [-6.0, -3.0, 0.0]
    high_gain_vals = [-6.0, -3.0, 0.0]
    transient_vals = [0.0, 0.3, 0.6]
    drc_vals = [0.0, 0.4, 0.8]
...
def _detect_embedding_columns(cols: List[str]) -> List[str]:
    emb_cols = [c for c in cols if re.match(r"^yamnet_\d{1,4}$", c)]
    if len(emb_cols) >= 256:
        emb_cols.sort(key=lambda s: int(s.split("_")[1]))
        return emb_cols
    return []
...
def _inject_best_features(feat_dict: Dict[str, float], cand: Dict[str, float]) -> None:
    feat_dict["low_gain_db"] = float(cand.get("low_gain_db", 0.0))
    feat_dict["mid_gain_db"] = float(cand.get("mid_gain_db", 0.0))
    feat_dict["high_gain_db"] = float(cand.get("high_gain_db", 0.0))
    feat_dict["transient_smooth"] = float(cand.get("transient_smooth", 0.0))
    feat_dict["drc_strength"] = float(cand.get("drc_strength", 0.0))

    feat_dict["best_low_gain_db"] = feat_dict["low_gain_db"]
    feat_dict["best_mid_gain_db"] = feat_dict["mid_gain_db"]
    feat_dict["best_high_gain_db"] = feat_dict["high_gain_db"]
    feat_dict["best_transient_smooth"] = feat_dict["transient_smooth"]
    feat_dict["best_transient_amount"] = feat_dict["transient_smooth"]
    feat_dict["best_drc_strength"] = feat_dict["drc_strength"]
    feat_dict["best_compressor_strength"] = feat_dict["drc_strength"]
...
def _score_candidates_with_ranker(
    ...
    debug = {
        ...
        "missing_features_filled_with_zero": int(missing_cols),
    }
...
def _ensure_mono_wav_for_scoring(audio_path: Path) -> Path:
    ...
    mono = y[:, 0] if y.shape[1] == 1 else np.mean(y[:, :2], axis=1)

    if mono.size < 256:
        raise RuntimeError(f"Audio too short for scoring: {audio_path}")
...
def main():
    ...
    ap.add_argument("--controller_model", default=str(DEFAULT_CONTROLLER_MODEL))
    ap.add_argument("--controller_cols", default=str(DEFAULT_CONTROLLER_COLS))

    ap.add_argument("--ctrl_lookup_csv", default=str(DEFAULT_CTRL_LOOKUP),
                    help="Path to controller_dataset_yamnet_m003.csv for runtime feature merge.")

    ap.add_argument("--trim_seconds", type=float, default=0.0)
    ap.add_argument("--demucs_required", action="store_true")

    ap.add_argument("--lofi_layers_dir", default=str(ROOT / "data" / "lofi_layers_prepped"))
    ap.add_argument("--lofi_enabled", type=int, default=1)
    ap.add_argument("--lofi_gain_db", type=float, default=0.0)
    ...
    if not lofi_layers_dir.exists():
        raise RuntimeError(f"Missing lofi layers dir: {lofi_layers_dir}")

    ctrl_map = _load_ctrl_lookup(ctrl_lookup_csv) if ctrl_lookup_csv else {}
    ...
    feat_dict, emb_meta = _fill_controller_feature_dict(cols, base_feats, work_wav)

    tk = _norm_track(in_path.name)
    lookup_row = ctrl_map.get(tk, {})
    _merge_lookup_features(feat_dict, cols, lookup_row)

    booster, xgb = _load_xgb_model(controller_model)
    candidates = _candidate_grid()
    best_idx, best_score, debug_meta = _score_candidates_with_ranker(booster, xgb, cols, feat_dict, candidates)
    ...
    y_base_stereo = np.concatenate([y_dsp, y_dsp], axis=1) if y_dsp.shape[1] == 1 else y_dsp[:, :2]
    ...
    sim = float(np.dot(emb_before, emb_after) / den) if den > 0 else 0.0
    ...
    meta = {
        ...
        "ctrl_lookup_csv": str(ctrl_lookup_csv) if ctrl_lookup_csv else "",
        "ctrl_lookup_found": bool(lookup_row),
        "controller_cols_count": len(cols),
        "instrumental_wav": str(work_wav),
        "vocals_removed_method": vocals_meta.get("vocals_removed_method", ""),
        **emb_meta,
        **debug_meta,
    }
